...
one.f.flows = {
    id : {
        ...
        modal : {
            ...
            form : {
                ...
                // add queueid/minRate/maxRate, lbc, 0802
                queueId : "one_f_flows_id_modal_form_queueid",
                minRate : "one_f_flows_id_modal_form_minRate",
                maxRate : "one_f_flows_id_modal_form_maxRate",
    ...
    dashlet : function($dashlet, callback) {
        ...
        one.f.flows.ajax.dashlet(function($table) {
            ...
            if (one.f.flows.registry.privilege === 'WRITE') {
                ...
                // add queue button, lbc, 0802
                var btnaddq = one.lib.dashlet.button.single("Add Queue", one.f.flows.id.dashlet.addq, "btn-primary", "btn-mini");
                var $button = one.lib.dashlet.button.button(button);
                var $btnaddq = one.lib.dashlet.button.button(btnaddq);
                ...
                $btnaddq.click(function() {
                	var $modal = one.f.flows.modal.initaddq();
                	$modal.modal();
                });
                $dashlet.append($button);
                $dashlet.append($btnaddq);
    ...
    modal : {
        ...
        // add queue button initialize(), lbc, 0802
        initaddq : function() {
        	var h3 = "Add Queue";
            var footeraddq = one.f.flows.modal.footeraddq();
            var $modal = one.lib.modal.spawn(one.f.flows.id.modal.modal, h3, "", footeraddq);
...
//            });
            ...
            $('#'+one.f.flows.id.modal.addq, $modal).click(function() {
                one.f.flows.modal.addq($modal, 'true');
            ...
            // inject body (nodePorts)
            one.f.flows.modal.ajax.nodes(function(nodes, nodeports) {
                var $body = one.f.flows.modal.bodyaddq(nodes, nodeports);
                one.lib.modal.inject.body($modal, $body);
            });
            return $modal;
        },
        // add queue post, lbc, 0802
        addq: function($modal,install) {
        	var result = {};
        	result['installInHw'] = install;
        	var resource = {};
            var nodeId = $('#'+one.f.flows.id.modal.form.nodes, $modal).val();
            var port = $('#'+one.f.flows.id.modal.form.port, $modal).val();
            var queueId = $('#'+one.f.flows.id.modal.form.queueId, $modal).val();
            var minRate = $('#'+one.f.flows.id.modal.form.minRate, $modal).val();
            var maxRate = $('#'+one.f.flows.id.modal.form.maxRate, $modal).val();
            resource['nodeId'] = nodeId;
            resource['port'] = port;
            resource['queueId'] = queueId;
            resource['minRate'] = minRate;
            resource['maxRate'] = maxRate;
            one.f.flows.modal.ajax.addqueue(resource);
        },
        ...
        ajax : {
            ...
            // add queue ajax, lbc, 0802
            addqueue : function(resource){
            	
            	$.post(one.f.address.root+one.f.address.flows.addq, resource, function(data) {
            		alert("addqueue");
            	});
            }
        },
        ...
        // add queue dialog body, lbc, 0802
        bodyaddq : function(nodes, nodeports) {
			...
			$fieldset.append($label).append($select);
			// queue id
			var $label = one.lib.form.label("Queue ID");
			var $input = one.lib.form.input("Queue ID");
			$input.attr('id', one.f.flows.id.modal.form.queueId);
			$fieldset.append($label).append($input);
			// minRate
			var $label = one.lib.form.label("Min Rate");
			var $input = one.lib.form.input("Min Rate");
			$input.attr('id', one.f.flows.id.modal.form.minRate);
			$fieldset.append($label).append($input);
			// maxRate
			var $label = one.lib.form.label("Max Rate");
			var $input = one.lib.form.input("Max Rate");
			$input.attr('id', one.f.flows.id.modal.form.maxRate);
			$fieldset.append($label).append($input);
			// append & return form
...
//                	});
        ...
        // add footer for qos, lbc, 0802
        footeraddq : function() {
        	var footer = [];
        	// add button
        	var addButton = one.lib.dashlet.button.single("Add Queue", one.f.flows.id.modal.addq, "btn-success","");
        	var $addButton = one.lib.dashlet.button.button(addButton);
        	footer.push($addButton);
        	// close button
